...
import com.amao.gmall.constant.EsConstant;
import com.amao.gmall.pms.entity.*;
import com.amao.gmall.pms.mapper.*;
...
public class ProductServiceImpl extends ServiceImpl<ProductMapper, Product> implements ProductService {
    ...
    public void saveProduct(PmsProductParam productParam) {
        ...
        //2）、pms_sku_stock：sku_库存表
        proxy.saveSkuStock(productParam);

        //3）、pms_product_attribute_value：保存这个商品对应的所有属性的值
        proxy.saveProductAttributeValue(productParam);

        //4）、pms_product_full_reduction：保存商品的满减信息
        proxy.saveFullReduction(productParam);

        //5）、pms_product_ladder：满减表
    ...
    private void saveProductToEs(Long id) {
        ...
        QueryWrapper<SkuStock> skuStockWrapper = new QueryWrapper<SkuStock>().eq("product_id", id);
        List<SkuStock> stocks = skuStockMapper.selectList(skuStockWrapper);
        ...
        log.debug(attributeValues.toString());
        ...
        try {
            ...
            if (succeeded){
                ...
                log.error("ES中，id为{}商品数据为保存成功，开始重试",id);
            }
        } catch (IOException e) {
